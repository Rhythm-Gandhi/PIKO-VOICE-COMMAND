...
describe("history-based smart suggestions",()=>{
  const now=new Date("2026-08-24T00:00:00Z");
  const bought=(productId:string,date:string,overrides:Partial<HistoryItem>={})=>({...makeItem(catalog.find(p=>p.id===productId)!.name,1,catalog.find(p=>p.id===productId)!.unit,"manual",catalog.find(p=>p.id===productId)),purchasedAt:`${date}T00:00:00Z`,...overrides}) as HistoryItem;
  const history=(productId:string,dates:string[])=>dates.map(date=>bought(productId,date));

  it.each([["harvest-bread","bread"],["mother-milk","milk"],["colgate-toothpaste","toothpaste"]])("recommends arbitrary repeated %s purchases",(productId,name)=>{const result=suggestions({...defaultData,history:history(productId,["2026-08-10","2026-08-17"])},8,now);expect(result.find(s=>s.kind==="history")).toMatchObject({product:{normalizedName:name}})});
  it("does not infer running low from one purchase",()=>expect(suggestions({...defaultData,history:history("harvest-bread",["2026-08-17"])},8,now).some(s=>s.kind==="history")).toBe(false));
  it("calculates the median cadence from three purchases",()=>{const result=suggestions({...defaultData,history:history("mother-milk",["2026-07-31","2026-08-09","2026-08-17"])},8,now),repeat=result.find(s=>s.kind==="history");expect(repeat?.medianIntervalDays).toBe(8.5);expect(repeat?.reason).toContain("every 9 days")});
  it("does not recommend a repeat purchase before it is due",()=>expect(suggestions({...defaultData,history:history("mother-milk",["2026-08-14","2026-08-21"])},8,now).some(s=>s.kind==="history")).toBe(false));
  it("ranks a more overdue repeat above a less overdue repeat",()=>{const result=suggestions({...defaultData,history:[...history("harvest-bread",["2026-08-01","2026-08-08"]),...history("mother-milk",["2026-08-10","2026-08-17"])]},8,now).filter(s=>s.kind==="history");expect(result.map(s=>s.product.normalizedName)).toEqual(["bread","milk"])});
  it("excludes cart and dismissed repeat suggestions",()=>{const rows=history("mother-milk",["2026-08-10","2026-08-17"]),data={...defaultData,history:rows,list:[makeItem("milk")],dismissed:["history-mother-milk"]};expect(suggestions(data,8,now).some(s=>s.product.normalizedName==="milk")).toBe(false);expect(suggestions({...data,list:[]},8,now).some(s=>s.kind==="history")).toBe(false)});
  it("uses an available substitute when a repeated catalog product is unavailable",()=>{const milk=catalog.find(p=>p.id==="mother-milk")!,available=milk.inStock;try{milk.inStock=false;expect(suggestions({...defaultData,history:history("mother-milk",["2026-08-10","2026-08-17"])},8,now).find(s=>s.kind==="history")).toMatchObject({product:{inStock:true},reason:expect.stringContaining("available substitute")})}finally{milk.inStock=available}});
  it("uses repeated brands and categories as lightweight preferences",()=>{const rows=[bought("colgate-brush","2026-08-20"),bought("colgate-toothpaste","2026-08-21")],result=suggestions({...defaultData,history:rows},8,now);expect(result.some(s=>s.kind==="preference"&&s.product.brand==="Colgate")).toBe(true);expect(result.some(s=>s.kind==="preference"&&s.product.category==="Personal Care")).toBe(true)});
  it("keeps seasonal and sale suggestions",()=>{const result=suggestions(defaultData,5,now);expect(result.some(s=>s.kind==="seasonal")).toBe(true);expect(result.some(s=>s.kind==="sale")).toBe(true)});
  it("recommends custom history without converting it to a catalog product",()=>{const custom={...makeItem("Quinoa",500,"g"),name:"Quinoa",normalizedName:"quinoa",category:"Grains & Flours",purchasedAt:"2026-08-10T00:00:00Z"} as HistoryItem,rows=[custom,{...custom,id:crypto.randomUUID(),purchasedAt:"2026-08-17T00:00:00Z"}],repeat=suggestions({...defaultData,history:rows},8,now).find(s=>s.kind==="history");expect(repeat).toMatchObject({product:{normalizedName:"quinoa",custom:true},quantity:500,unit:"g"})});
  it("is read-only until the user adds a suggestion",()=>{const data={...defaultData,history:history("harvest-bread",["2026-08-10","2026-08-17"])},before=serializeData(data),repeat=suggestions(data,8,now).find(s=>s.kind==="history")!;expect(serializeData(data)).toBe(before);const added=mergeItem(data.list,makeItem(repeat.product.name,repeat.quantity,repeat.unit,"suggestion",repeat.product));expect(added).toHaveLength(1)});
  it("loads existing saved history without loss",()=>{const data={...defaultData,history:history("mother-milk",["2026-08-10","2026-08-17"]),dismissed:["history-harvest-bread"]},loaded=loadData(serializeData(data));expect(loaded.history).toHaveLength(2);expect(loaded.dismissed).toEqual(data.dismissed)});
});
